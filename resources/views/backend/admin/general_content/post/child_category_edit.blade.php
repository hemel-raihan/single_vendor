...
<ul class="transfer-double-group-list-li-ul transfer-double-group-list-li-ul-1636878492751">
    @foreach($sub_category->childrenRecursive as $key => $sub)
        ...
        <div class="checkbox-group">
                ...
                <input type="checkbox" name="categories[]" value="{{$sub->id}}" @foreach($post->contentcategories as $category) {{$sub->id == $category->id ? 'checked' : ''}} @endforeach class="checkbox-normal group-checkbox-item-1636878492751 belongs-group-0-1636878492751" id="group_<?php echo $sub_category->id; ?>_checkbox_<?php echo $key ?>_1636878492751" />
            ...
            @include('backend.admin.general_content.post.child_category_edit', ['sub_category' => $sub,'post' => $post])
            @endif
         </li>
         @endforeach
</ul>
