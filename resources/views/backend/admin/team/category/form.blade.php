...
@section('content')
						...
						<div class="page-header">
							<div>
								<h1 class="page-title">{{ isset($teamcategory) ? 'Edit ' : 'Create '}}Categories</h1>
								{{-- <ol class="breadcrumb">
							...
							<div class="ms-auto pageheader-btn">
								<a href="{{route('admin.teamcategories.index')}}" class="btn btn-primary btn-icon text-white me-2">
									<span>
                   ...
                   <!-- ROW-1 OPEN -->
    <form method="POST" action="{{isset($teamcategory) ? route('admin.teamcategories.update',$teamcategory->id) : route('admin.teamcategories.store')}}" enctype="multipart/form-data">
    @csrf
    @isset($teamcategory)
    @method('PUT')
    @endisset
			...
			<div class="card">
				...
				<div class="card-body">
					...
					<div class="form-group">
						...
						<input type="text" class="form-control @error('name') is-invalid @enderror" value="{{$teamcategory->name ?? old('name')}}" name="name" id="exampleInputname" placeholder="Category Name">
                        @error('name')
				...
				<div class="card-footer text-end">
					...
					<a href="{{route('admin.teamcategories.index')}}" class="btn btn-danger mt-1">Cancel</a>
				</div>
			</div>
		</div>
                                            ...
                                            @foreach($categories as $key => $categoryy)
												...
												<li class="transfer-double-group-list-li transfer-double-group-list-li-1636878492751">
													<div class="checkbox-group">
														<input type="checkbox" name="parent_id" value="{{$categoryy->id}}" {{$categoryy->id == $teamcategory->parent_id ? 'checked' : ''}} class="checkbox-normal group-select-all-1636878492751" id="group_{{$key}}_1636878492751" /><label for="group_{{$key}}_1636878492751" class="group-name-1636878492751">{{$categoryy->name}}</label>
													</div>
                                                    @if($categoryy->childrenRecursive->count()>0)


													  @include('backend.admin.team.category.child_category_edit', ['sub_category' => $categoryy,'category' => $teamcategory])
                                            ...
                                            @foreach($categories as $key => $category)
												...
												<li class="transfer-double-group-list-li transfer-double-group-list-li-1636878492751">
                                                    ...
                                                    @if($category->childrenRecursive->count()>0)
													  ...
													  @include('backend.admin.team.category.child_categories', ['sub_category' => $category])
				...
				<div class="card-body">
					...
					<div class="form-group">
						...
						<label class="custom-switch">
							<input type="checkbox" name="status" {{$teamcategory->status == true ? 'checked' : ''}} class="custom-switch-input ">
